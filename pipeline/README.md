...
These subtrees are generated by pruning braches from the gene tree so that at most one sequence per species is present. 
...
For this, we use Wagner pasimony implemented in [COUNT] program [Csuros 2008] (which is called by [ancestral_content.py] script).
