...
$utils/get_options_gb "  -t=d -b1=50 -b2=50 -b5=a  " ./$file.$ext > ./options
set stexec = $status
echo Statut : $stexec
if ($stexec > 0 ) then
...
$utils/convert_phylip $file.$ext-gb
set stexec = $status
echo Statut : $stexec

echo "Copie de l'alignement traite"
ls
echo "\cp ./$file.$ext-gb.phyl $gbresults/"
\cp ./$file.$ext-gb.phyl $gbresults/
set stexec = $status
echo Statut : $stexec
if ($stexec > 0 ) then
...
set phymlopt = "-i $file.$ext-gb.phyl -d nt -q -b -3  -m GTR -v e -c 8 -a e -s BEST"
echo "phyml $phymlopt"
...
\cp ./$file.$ext-gb.phyl_phyml_tree.txt $treeresults/$file.phb
set stexec = $status
echo Statut : $stexec
if ($stexec > 0 ) then
